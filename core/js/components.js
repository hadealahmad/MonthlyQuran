...
const UIComponents = {
  ...
  createTaskCard(
    ...
    const currentDate =
      window.UI && window.UI.currentDate
        ? DateUtils
      ...
      if (parts.length >= 3) {
        ...
        if (!isNaN(itemNumber)) {
          ...
          if (
            ...
            size > 1 &&
            config &&
            config.start_page != null
          ) {
            const startPage =
              (config.start_page || 1) + (itemNumber - 1) * size;
            const endPage = startPage + size;
            title.textContent = Algorithm.formatPageRangeLabel(
              idUnitType,
              startPage,
              endPage,
            );
            ...
            const actualNumber =
              typeof Algorithm.calculateActualUnitNumber === "function"
                ? Algorithm.calculateActualUnitNumber(
                    idUnitType,
                    itemNumber,
                    config,
                  )
                : itemNumber;
            title.textContent = Algorithm.formatContentReference(
              idUnitType,
              actualNumber,
              config,
            );
    ...
    if (isOverdue) {
      ...
      const PRIORITY_YESTERDAY =
        (window.PRIORITY && window.PRIORITY.YESTERDAY) || 2;
    ...
    const ariaLabel = isCompleted
      ? i18n.t("aria.markIncomplete")
      : i18n.t("aria.markComplete");
    checkbox.setAttribute("aria-label", ariaLabel);

    // Use SVG utils instead of innerHTML
    if (typeof SVGUtils !== "undefined") {
      const svgIcon = isCompleted
        ? SVGUtils.createCheckboxChecked()
        : SVGUtils.createCheckboxUnchecked();
    ...
    checkbox.addEventListener("click", async (e) => {
      ...
      const uiDate =
        window.UI && window.UI.currentDate ? window.UI.currentDate : new Date();
      const currentDateStr =
        typeof DateUtils !== "undefined" && DateUtils.getLocalDateString
      ...
      const checkDate =
        (isOverdue || isCatchup) && originalDueDate
          ? originalDueDate
          : currentDateStr;
      const currentlyCompleted = await Storage.isReviewCompleted(
        item.id,
        station,
        checkDate,
      );
      ...
      setTimeout(async () => {
        ...
        if (currentlyCompleted) {
          ...
          if (isCatchup) {
            await Storage.unmarkBacklogItemComplete(
              item.id,
              station,
              currentDateStr,
            );
          ...
          if (isCatchup) {
            await Storage.markBacklogItemComplete(
              item.id,
              station,
              currentDateStr,
            );
  ...
  createProgressTimelineItem(item, config = null) {
    ...
    const memDate = DateUtils
      ? DateUtils.normalizeDate(item.date_memorized)
      : new Date(item.date_memorized);
    ...
    const daysPassed = DateUtils
      ? DateUtils.daysDifference(today, memDate)
      : Math.floor((today - memDate) / (1000 * 60 * 60 * 24));
    ...
    for (let i = reviewDates.length - 1; i >= 0; i--) {
      const reviewDateObj = DateUtils
        ? DateUtils.normalizeDate(reviewDates[i].date)
        : (() => {
            const d = new Date(reviewDates[i].date);
            d.setHours(0, 0, 0, 0);
            return d;
          })();
      const reviewKey = `${reviewDates[i].station}-${reviewDates[i].date}`;
      const isCompleted =
        item.reviews_completed && item.reviews_completed.includes(reviewKey);
    ...
    if (nextReviewDate) {
      ...
      const nextReviewText = i18n.t("progress.nextReview", {
        date: new Date(nextReviewDate.date).toLocaleDateString(),
      });
      nextReview.textContent = nextReviewText.replace(
        /\{\{date\}\}/g,
        new Date(nextReviewDate.date).toLocaleDateString(),
      );
      meta.appendChild(nextReview);
    }
    const currentStationText = document.createElement("div");
    const stationText = i18n.t("progress.currentStation", {
      station: currentStation,
    });
    currentStationText.textContent = stationText.replace(
      /\{\{station\}\}/g,
      currentStation,
    );
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      const isCompleted =
        item.reviews_completed && item.reviews_completed.includes(reviewKey);
      const reviewDateObj = DateUtils
        ? DateUtils.normalizeDate(reviewDate.date)
        : (() => {
            const d = new Date(reviewDate.date);
            d.setHours(0, 0, 0, 0);
            return d;
          })();
      const isPast = reviewDateObj <= today;

      const indicator = this.createStationIndicator(
        i,
        isCompleted,
        i === currentStation,
        !isPast && i > currentStation,
      );
      ...
      indicator.setAttribute(
        ...
        i18n.t(`stations.${i}`) +
          (isCompleted
            ? " – " + (i18n.t("progress.completed") || "Completed")
            : ""),
      );
      ...
      indicator.addEventListener("click", () => {
        ...
        if (!wasExpanded) {
          const detailRow = this.createStepDetailRow(
            payload.item,
            payload.reviewDate,
            payload.isCompleted,
            payload.config,
  ...
  createStepDetailRow(item, reviewDate, isCompleted, config = null) {
    ...
    const dateStr = new Date(reviewDate.date).toLocaleDateString(undefined, {
      dateStyle: "medium",
    });
    ...
    if (isCompleted) {
      ...
      const unitSize =
        config && config.unit_size != null ? config.unit_size : null;
      if (navigator.onLine && unitType === "page") {
        let pageNumber = null;
        const fractionalMatch =
          item.content_reference &&
          item.content_reference.match(/(\d+)([½¼¾])/);
        if (fractionalMatch) {
          ...
          const decimalMatch =
            item.content_reference &&
            item.content_reference.match(/(\d+\.?\d*)/);
          if (decimalMatch) pageNumber = parseFloat(decimalMatch[1]);
          else {
            const intMatch =
              item.content_reference && item.content_reference.match(/\d+/);
  ...
  createScheduleItem(item, stationNumber = null, isCompleted = false) {
    ...
    checkbox.addEventListener("click", async (e) => {
      // Determine current completion status for animation
      const dateInput = document.getElementById("current-date");
      const currentDateStr = dateInput
        ? dateInput.value
        : new Date().toISOString().split("T")[0];
      const station = stationNumber || 1;
      const currentlyCompleted = await Storage.isReviewCompleted(
  ...
  async showReadingModal(
    ...
    const title = document.createElement("h3");
    title.style.cssText =
      "font-size: 1.125rem; font-weight: 600; margin: 0; color: var(--fg);";
    const size = unitSize != null ? parseFloat(unitSize) : 1;
    ...
    const isCompleted = await Storage.isReviewCompleted(
      itemId,
      stationNumber,
      date,
    );
    ...
    try {
      ...
      if (unitType === "page") {
        ...
        content.textContent =
          'Units other than "Page" are coming soon to the reader.';
        return;
      }
      ...
      if (textData && textData.data && textData.data.ayahs) {
        ...
        // btn for comparing between play and pause
        ...
        textData.data.ayahs.forEach((ayah) => {
          ...
          playBtn.onclick = async () => {
            const surah = pad(ayah.surah.number);
            const ayahNum = pad(ayah.numberInSurah);
          ...
          //adding the btn before the ayah number
          ayahSpan.appendChild(playBtn);
  ...
  createConsistencyMap(items) {
    ...
    for (let week = 0; week < totalWeeks; week++) {
      ...
      if (!monthPositions.has(month + "-" + weekStart.getFullYear())) {
        monthPositions.set(month + "-" + weekStart.getFullYear(), {
          col: week,
          month,
        });
      }
    ...
    const showTooltip = (cell, date, count) => {
      const dateStr = date.toLocaleDateString(
        i18n.currentLanguage === "ar" ? "ar-SA" : "en-US",
        {
          day: "numeric",
          month: "short",
          year: "numeric",
